var CalcPrecedence = (function () {
    ...
    CalcPrecedence.prototype.calc = function (str) {
        this.expression = str;
        //split(*) -> 7+12*5/2 as 
        ...
        for (var i = 0; i < this.valueArray.length; i++) {
            if (this.valueArray[i] == '-') {
                if (parseFloat(this.valueArray[i + 1]) > 0) {
                    ...
                    var negative = -1 * parseFloat(this.valueArray[i + 1]);
    ...
    CalcPrecedence.prototype.inputExpr = function (strin) {
        ...
        var count = 0;
        for (var i = 0; i < this.parantheStr.length; i++) {
        ...
        if (count == 0) {
            ...
            for (var i = 0; i < this.parantheStr.length; i++) {
                if (this.parantheStr[i] == '(') {
                    for (var j = i + 1; j < this.parantheStr.length; j++) {
    ...
    CalcPrecedence.prototype.updateString = function () {
        var substr = this.parantheStr.substring(this.open + 1, this.close);
        //console.log(substr)
        var returnedValue = this.calc(substr);
        //updated inputExpr
        var charArray = this.parantheStr.split("");
